...
“candidate 已实现并进入最终验证”。当前复核基线是 `origin/master=90f3fc6b`；此前 `14606afc` 只称
branch-cut/探索性基线。为保证 baseline/candidate 使用相同 helper，已在该 master 上构造并独立复核
synthetic P0-only baseline。随后发现 latest master 的 proc namespace magic-link 在 bind 时丢失 mount
projection；该 correctness 修复同时进入双方，最终 baseline 为 `fc9fe357`，candidate 核心提交为
`e36127da..02036938`，避免把“baseline 无法启动”冒充性能收益。
...
manifest。host transcript、runner 负测和 study 单测已经通过；P0 baseline clean artifact 已封印，candidate
必须在最终文档提交和最新 master 复核后重新封印，正式采样尚未完成。
