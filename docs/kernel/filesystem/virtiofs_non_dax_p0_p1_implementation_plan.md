...
用户已批准实施 P0、P1；第 6、7 节的“未授权/停止”文字是历史记录。当前实现状态不是“验收完成”，而是
“candidate 已实现并进入最终验证”。最终分支将纳入复核时最新 `origin/master=772fa84f`；此前
`14606afc` 只称 branch-cut/探索性基线。当前 candidate 已 rebase 到 `772fa84f` 并完成 clean build。
...
manifest。host transcript、runner 负测和 study 单测已经通过；clean candidate 已封印，正式采样尚未完成。
...
focused guest stats dunitest、random 和 concurrent smoke 已通过。仍未通过的门禁：20 次 read
correctness；EOF/truncate/invalidate/disconnect/teardown；拒绝 `FUSE_ASYNC_READ` 的 guest runtime；mmap；
off/light/off 与 master/candidate/master；Linux guest reference；CPU/MiB；CubeSandbox 正式
baseline/candidate。CubeSandbox candidate smoke 已用本轮 kernel 启动并复现 `/bin/busybox` 读取仅
2.6--2.9 MB/s；sandbox/log/销毁证据已收集且测试机原 kernel 已恢复，但该 smoke 仍混有 overlayfs、
applet exec 和辅助命令，不可替代同 schema A/B。尤其 mmap 修改了共用 fill/page-cache 路径，未解除前不得宣称 P1 correctness 或总体
non-DAX 验收通过。串口批量输入造成的 TTY spinlock stall 是独立非 FUSE 事件。
