...
| host tests、Clippy、kernel/user/image build | 已通过（当前基线纳入 `origin/master=33681d8e`） |
| synthetic P0-only baseline | 已独立复核；加入双方共用的 mount correctness 后封印为 `2a8f5e25` |
...
`origin/master=33681d8e` 加同一套 P0 观测/benchmark 工具和双方共用的 mount/path correctness 修复形成
synthetic baseline `2a8f5e25`；P1 candidate 为 `e8aae407..9b8b77b9`。两者使用相同 helper SHA，且
candidate 在最终文档提交及
...
逐请求时间线闭环。Linux reference 已完成；正式本地结论仍必须等待 A1>=9/B>=9/A2>=9、CPU/MiB 和
payload 分位数。
...
`ae73725b8d77dd7875e55316972796dac474b405fd78417662fa41cb70b4997d` 已恢复。上述 busybox smoke 只保留为
发现问题的历史证据。正式 Cube adapter 已把同一 helper 注入 guest，并以
单实例、同镜像、同 request、同 non-DAX 配置完成 P0/P1 双包 seal+verify；远端原 kernel/image 最终按
SHA、属主、权限和大小恢复，running sandbox、loop 与 mount 均为零。
...
1. queue/dispatch/processing/transport/reservation owner current 已变为 always-on，并覆盖 Drop、abort、
   disconnect 和 reset 路径；仍须用同 revision 的 off/light/off 重复实验量化成本。逐 opcode、READ 桶和
   direct-DMA 字节等可选 totals 在 off 下不保证完整。
...
collector QEMU/virtiofsd/peer worker PID+starttime 的绑定已经加入 host 侧协议。clean-tree build
attestation、dirty-tree 拒绝、off quiescence、Linux/Cube adapter 及其破坏性负测也已实现；剩余工作是
在冻结 artifact 上执行正式样本，而不是补写手工汇总。
