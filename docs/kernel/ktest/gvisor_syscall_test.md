...
```
   ...
   Use ``./gvisor-test-runner` to run specific test cases.
...
The test framework enables whitelist mode by default, running only the test programs specified in ``whitelist.txt`. This allows for gradual validation of DragonOS's system call implementation.

## Blacklist Filtering

For each test program, specific test cases can be blocked through files in the ``blocklists/` directory. This is particularly useful for skipping tests that are not currently supported or are unstable.
...
```
   ...
   This command downloads the test suite, compiles DragonOS, writes to the image, and automatically runs the tests. After the first execution, the test cases are installed in the ``/opt/tests/gvisor` directory of DragonOS.
...
```
   ...
   The test case code is located in the `test/syscalls/linux` directory. Cases are written with gtest; each syscall has one or more `.cc` files.
...
1. **Copy Test Case Source Code**
   ...
   From the cloned gvisor repository, find the corresponding test case source code file (usually ``<test_name>.cc`), and copy it to the DragonOS project root directory:
...
```
   ...
   ```bash
   cp gvisor/test/syscalls/linux/socket_test.cc /path/to/DragonOS/
```

2. **Use AI Assistant for Repair**

   Provide the copied ``.cc` file to an AI assistant (such as Cursor's AI function), and explain:
...
1. **Edit the Whitelist File**
   ...
   Open the ``user/apps/tests/syscall/gvisor/whitelist.txt` file and add the test program name:
...
2. **Create a Blocklist File**
   ...
   Create a file with the same name as the test program in the ``user/apps/tests/syscall/gvisor/blocklists/` directory:
...
4. **Format Explanation**
   ...
   - Supports wildcard ``*` to match any characters
   - Test case names typically follow the format ``TestSuite.TestCase`
...
5. **Examples**
   ...
   - ``user/apps/tests/syscall/gvisor/blocklists/socket_test` - Blacklist for socket tests
   - ``user/apps/tests/syscall/gvisor/blocklists/epoll_test` - Blacklist for epoll tests

Complete Workflow Example
--------------

The following is a complete repair workflow example, using ``open_test` as an example:
...
3. **Fix the Code**
   ...
   cp gvisor/test/syscalls/linux/open_test.cc /path/to/DragonOS/
```
...
- **Code Quality**: Ensure repaired code meets DragonOS coding standards; run ``make fmt` for formatting
