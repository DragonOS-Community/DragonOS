...
- Translation time: 2026-06-06 09:33:23
...
- **`mountinfo`**: Modern format containing mount ID, parent-child relationships, propagation flags, superblock options, etc.
...
Propagation types (`shared` / `master` / `propagate_from` / `unbindable`) **only appear in the optional fields of `mountinfo`** and are not written to the regular options column of `/proc/*/mounts`.

## 2. Functional Roles of Each Interface

### 2.1 `/proc/mounts` and `/proc/self/mounts`

`/proc/mounts` is implemented as a **symbolic link pointing to `self/mounts`** (the result of `readlink` is `self/mounts`), and resolving it is equivalent to reading `/proc/self/mounts`, which is the mount list of the **current reading process** within its mount namespace and `fs root`.
...
If the underlying filesystem returns additional content via `proc_show_mount_stats`, it is appended at the end of the same line. The permissions are **0400** (readable only by the owner), different from the 0444 of `mounts` / `mountinfo`.

## 3. Kernel Source Code Layout

The mount export logic is centralized in **`kernel/src/filesystem/procfs/mount/`**, no longer using the historical single-file implementation of `mount_view.rs` or scattered implementations like `procfs/mounts.rs` and `pid/mountinfo.rs`.
...
- `kernel/src/filesystem/procfs/pid/mod.rs`: In `PidDirOps::STATIC_ENTRIES`, a `MountProcFileOps` is registered for each of `mountinfo` / `mounts` / `mountstats` (distinguished by `ProcMountRenderKind` for format)
...
1. **`open()`** (opening `MountProcFileOps::open` or opening `/proc/self/mounts` via symlink)
   - Resolves the target: `ProcPidTarget` → thread group leader `ProcessControlBlock`
2. **`collect_visible_mounts()`** (processing `collect.rs`)
   - Traverses the target `mnt_ns.mount_list()`, sorted by mount ID
   - Performs **visibility pruning** (using `visible_mountpoint`) based on the target `fs_struct.root()`
3. **`MountProcFields::from_entry()`** (snapshotting `fields.rs`)
...
Thus, the current model is: **generate a full file snapshot when opened, and no longer re-traverse the mount tree during subsequent reads within the same open session**.

### 4.2 Target Process Perspective

- `/proc/mounts` → `self/mounts` → `mounts` under the pid directory of the current process
- `/proc/[pid]/mounts|mountinfo|mountstats` binds fixedly to the namespace and root of the corresponding thread group of the target `pid` when opened
...
- When the target is under a restricted root like chroot, only mounts within that root subtree are retained, and the displayed path is normalized to a view rooted at `/`
...
High compatibility, fewer fields; **does not include** propagation flags. Like Linux, the current process view should be accessed via the `/proc/mounts` symlink.
...
- Content within the same `open()` is a snapshot; reopening `open()` shows updated mount collections and statistics
...
### 6.1 Overall Difference Overview

| Dimension | Linux | DragonOS Current Implementation |
|------|-------|-------------------|
| Opening Method | `seq_file` + Iterator | One-time rendering and caching when `open()` |
...
| Perspective Binding | Target task's `mnt_ns + fs root` | Same as above (as `collect_visible_mounts`) |
...
Linux uses `mounts_open_common()` + `seq_file` to iterate the mount list during reading. DragonOS chooses to concatenate the full string at open time and cache it, resulting in simpler implementation, stable results within the same fd, but not fully equivalent to Linux's iterative model.

### 6.3 `mounts` / `mountinfo` Poll in `poll`

Linux can perform `_translated_label__poll`/`epoll_en` on `mounts` / `mountinfo` via mount namespace events. DragonOS has not yet implemented namespace event sequence numbers and wait queues, so it cannot serve as a mount change notification source.

### 6.4 Dynamic Nature of `mountstats` and `poll`

Linux has no dedicated `mountstats` poll semantics; DragonOS also does not invent additional poll for `mountstats`. Statistics and topology changes are observed by reopening the file.

### 6.5 Visibility Pruning Semantics

Linux uses `seq_path_root` etc. for root pruning based on path objects. DragonOS currently compares **absolute path strings** with the target `fs root`, with the same general direction but still differing in detail from Linux's path object semantics.

### 6.6 Traversal and Authoritative Data Source

Linux uses the namespace-level mount list as the authoritative data source. DragonOS retrieves the table from `MntNamespace::mount_list()` and sorts it, rather than performing DFS on a single mount tree; if alignment with Linux's iteration order and event model is needed in the future, further evolution is required on the `MntNamespace` side.
...
Notes:
- User-space tools dependent on **mount namespace `poll` notifications** are not yet compatible
...
- **Options Semantics**: Propagation flags only in `mountinfo`; `MountFlags` and VFS hooks divide export responsibilities
