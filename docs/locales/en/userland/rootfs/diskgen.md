...
- Translation time: 2025-12-26 10:37:05
...
After defining the software packages, no further modifications are needed. Simply run `nix run .#rootfs-${target}` to build the RootFS.
...
TODO: Future scripts will allow a two-step process with the option to inject custom files (in addition to directly injecting them in sysconfig).
