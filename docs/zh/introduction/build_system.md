# 构建DragonOS {#_build_dragonos}

## 0.快速体验

如果您只是想快速体验DragonOS，而不需要进行本地开发，我们强烈推荐您使用 **DragonOS Playground** 进行云原生开发。这种方式**零配置、一键启动**，无需安装任何本地依赖，即可在云端体验DragonOS的最新nightly build版本。
...
**接着，请直接跳到[编译命令讲解 ](/zh/introduction/build_system.html#_build_system_command)进行阅读！**
...
在以下依赖项中，除了`docker-ce`和`Rust及其工具链`以外，其他的都能通过系统自带的包管理器进行安装。关于docker以及rust的安装，请看后文。
...
**关于编译命令的用法，请见：[编译命令讲解 ](/zh/introduction/build_system.html#_build_system_command)**
...
## 6.编译命令讲解 {#_build_system_command}

**关于如何配置 RootFS（`ROOTFS_MANIFEST`、base 镜像、包集合切换），请先阅读：[DADK RootFS Manifest 配置（非 Nix） ](/zh/userland/rootfs/dadk-rootfs-manifest.html#_dadk_rootfs_manifest)**
...
- 编译文档: `make docs` （需要 Node.js 22+，在 `docs/` 下执行 `npm install`）
