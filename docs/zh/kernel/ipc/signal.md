...
当进程A想发送信号给进程B的时候，使用`kill(pid, signal)`接口进行发送。然后陷入内核的`sys_kill()`函数中进行处理。然后内核将会把信号加入目标进程的pcb的`sigpending`中。
...
为避免信号相关路径出现锁顺序反转导致的死锁，必须遵循固定的锁顺序：  
**先获取 sighand（共享信号状态），再获取 sig_info（线程私有信号状态）**。  
任何路径都不得在持有 sig_info 锁时进入 sighand 锁。  
如需同时访问两者，请按上述顺序获取，并保持访问窗口尽可能短。
