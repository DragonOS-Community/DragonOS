...
### 2.1 FifoRunQueue
 ...
 `FifoRunQueue`是FIFO调度器的运行队列，每个CPU维护一个实例。
...
### 2.2 FifoScheduler
 ...
 `FifoScheduler`实现了`Scheduler` trait，提供FIFO调度策略的核心逻辑。
...
## 5. Demo功能
 ...
 通过`fifo_demo` feature可以启用演示功能（`kernel/src/sched/fifo_demo.rs`），该功能创建一个FIFO调度的内核线程：
