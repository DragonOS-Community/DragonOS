...
use quote::quote;
use syn::{
    __private::ToTokens, parse::{self, Parse, ParseStream}, spanned::Spanned, token::Type, ItemFn, ItemTrait, Path
};
use uuid::Uuid;

/// 文件系统统一初始化宏,
/// 用于将函数注册到统一文件系统初始化列表中
///
/// ## 用法
///
/// ```rust
/// use system_error::SystemError;π
...
pub fn filesystem_manager(args: TokenStream, input: TokenStream) -> TokenStream {
    do_collect_init(args, input)
        .unwrap_or_else(|e| e.to_compile_error().into())
        .into()
}

fn do_collect_init(args: TokenStream, input: TokenStream) -> syn::Result<proc_macro2::TokenStream> {
    // 解析属性数
...
/// fn xxx() -> Result<Box<dyn FileSystem>, SystemError> {
///     Ok(())
/// }
/// ```
        ...
        if let syn::Type::Path(type_path) = output_type {
            if type_path.path.segments.last().unwrap().ident == "Result" {
                    ...
                    type_path.path.segments.last().unwrap().arguments.clone()
                {
                    ...
                    {
                        if let Type::Path(inner_arg) = type_arg {
