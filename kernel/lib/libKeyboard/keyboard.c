...
int keyboard_analyze_keycode(char* keycode)
{
    ...
    unsigned char vernier=0;

    int c = keyboard_get_scancode(keycode,vernier);
    // 循环队列为空
    if (c == -1)
        return 0;
    ...
    if (scancode == 0xE1) // Pause Break
    {
        ...
        // 清除缓冲区中剩下的扫描码
        for (int i = 1; i < 6; ++i)
            if (keyboard_get_scancode(keycode,vernier) != pause_break_scan_code[i])
            {
                key = 0;
                break;
            }
    ...
    {
        // 获取下一个扫描码
        scancode = keyboard_get_scancode(keycode,vernier);
        switch (scancode)
        {
        case 0x2a: // print screen 按键被按下
            if (keyboard_get_scancode(keycode,vernier) == 0xe0)
                if (keyboard_get_scancode(keycode,vernier) == 0x37)
                {
                    key = PRINT_SCREEN;
                    flag_make = true;
                }
        ...
        case 0xb7: // print screen 按键被松开
            if (keyboard_get_scancode(keycode,vernier) == 0xe0)
                if (keyboard_get_scancode(keycode,vernier) == 0xaa)
                {
                    key = PRINT_SCREEN;
                    flag_make = false;
                }
