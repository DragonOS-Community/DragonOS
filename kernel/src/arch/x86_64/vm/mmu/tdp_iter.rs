use crate::{arch::vm::mmu::mmu::gfn_round_for_level, mm::{virt_2_phys, PhysAddr, VirtAddr}, time::sleep, virt::kvm::host_mem::PAGE_SHIFT};

use super::{mmu::{PageLevel, PAGE_SIZE}, mmu_internal::KvmMmuPage};
...
impl TdpIter{
    pub fn start(&self,root_pt:usize,root_level:u8,min_level:u8,next_last_level_gfn:u64)->Self{
...
impl TdpIterInner{
    ///初始化ept iter
    pub fn start(&mut self,root_pt :usize,root_level:u8,
        min_level: u8,next_last_level_gfn:u64){
 ...
 */
    fn tdp_iter_refresh_sptep(&mut self){
        self.sptep = PhysAddr::new((self.pt_path[self.level as usize - 1] + 
