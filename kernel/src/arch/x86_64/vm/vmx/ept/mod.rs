...
impl EptPageMapper {
    ...
    pub fn table(&self) -> EptPageTable {
        EptPageTable::new(VirtAddr::new(0), self.root_page_addr, max_huge_page_level())
    }
    ...
    pub fn map_gpa(
        ...
        if !(gpa.check_aligned(MMArch::PAGE_SIZE) && hpa.check_aligned(MMArch::PAGE_SIZE)) {
            kerror!("Try to map unaligned page: gpa={:?}, hpa={:?}", gpa, hpa);
            return None;
        }
        ...
        loop {
            ...
            if table.level() == PageLevel::Level4K {
                ...
                if let Some(next_table) = next_table {
                    ...
                    let flags: PageFlags<MMArch> = unsafe {
                        PageFlags::from_data(
                            MMArch::ENTRY_FLAG_DEFAULT_TABLE | MMArch::ENTRY_FLAG_READWRITE,
                        )
                    };
