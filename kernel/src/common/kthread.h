...
// #define kthread_run_rt(thread_fn, data, name_fmt, ...) ({kdebug("kthread_run_rt:--------222--------"); struct process_control_block *__kt = kthread_create(thread_fn, data, name_fmt, ##__VA_ARGS__);__kt->policy=SCHED_RR;__kt->rt_se.time_slice=80; if (!IS_ERR(__kt)){kdebug("kthread_run_rt:--------222--------");kdebug("kthread_run_rt: %d", process_wakeup(__kt));kdebug("kthread_run_rt:--------2224--------");} __kt;})
