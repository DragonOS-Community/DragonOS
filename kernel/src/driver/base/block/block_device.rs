...
// impl<T: BlockDeviceAsBlock> Device for T {
//     fn dev_type(&self) -> DeviceType {
//         DeviceType::Block
//     }
// }
// impl<T: BlockDeviceAsBlock> BlockDevice for T {
...
//             if full {
//                 BlockDeviceAsBlock::write_at(self, range.lba_start, count, buf_slice)?;
//             } else {
//                 if self.blk_size_log2() > BLK_SIZE_LOG2_LIMIT {
//                     return Err(SystemError::E2BIG);
//                 }
...
//                 temp.resize(1usize << self.blk_size_log2(), 0);
//                 // 由于块设备每次读写都是整块的，在不完整写入之前，必须把不完整的地方补全
//                 BlockDeviceAsBlock::write_at(self, range.lba_start, 1, &mut temp[..])?;
//                 // 把数据从临时buffer复制到目标buffer
//                 temp[range.begin..range.end].copy_from_slice(&buf_slice);
//                 BlockDeviceAsBlock::write_at(self, range.lba_start, 1, &temp[..])?;
//             }
//         }
//         return Ok(len);
//         //self.0.lock().write_at(lba_id_start, count, buf)
...
//             if full {
//                 // 调用 BlockDevice::read_at() 直接把引用传进去，不是把整个数组move进去
//                 BlockDeviceAsBlock::read_at(self, range.lba_start, count, buf)?;
//             } else {
//                 // 判断块的长度不能超过最大值
//                 if self.blk_size_log2() > BLK_SIZE_LOG2_LIMIT {
//                     return Err(SystemError::E2BIG);
...
//                 let mut temp = Vec::new();
//                 temp.resize(1usize << self.blk_size_log2(), 0);
//                 BlockDeviceAsBlock::read_at(self, range.lba_start, 1, &mut temp[..])?;
...
//     fn sync(&self) -> Result<(), SystemError> {
//         return BlockDeviceAsBlock::sync(self);
//     }

//     fn blk_size_log2(&self) -> u8 {
//         BlockDeviceAsBlock::blk_size_log2(self)
//     }

//     fn as_any_ref(&self) -> &dyn Any {
//         BlockDeviceAsBlock::as_any_ref(self)
//     }

//     fn device(&self) -> Arc<dyn Device> {
//         BlockDeviceAsBlock::device(self)
//     }

//     fn block_size(&self) -> usize {
//         BlockDeviceAsBlock::block_size(self)
//     }

//     fn partitions(&self) -> Vec<Arc<Partition>> {
//         BlockDeviceAsBlock::partitions(self)
//     }
// }

// pub trait BlockDeviceAsBlock: KObject {
//     fn read_at(
//         &self,
