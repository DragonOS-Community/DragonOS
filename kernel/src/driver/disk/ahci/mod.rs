...
static mut locked_hba_port_list: SpinLock<Vec<Vec<&mut HbaPort>>> = SpinLock::new(Vec::new());
...
pub fn ahci_rust_init() -> Result<(), i32> {
    ...
    unsafe {
        ...
        // 全局数据 - 列表
        let mut hba_port_list = locked_hba_port_list.lock();
        ...
        for i in 0..(ahci_dev_counts as usize) {
            ...
            let ahci_port_base_vaddr = kmalloc(1048576, 0) as usize;
            ...
            let hba_mem = (virt_addr as *mut HbaMem).as_mut().unwrap();
            ...
            for j in 0..32 {
                if (pi >> j) & 1 > 0 {
                    let port = &mut hba_mem.ports[j];
                    let tp = port.check_type();
                    match tp {
                        HbaPortType::None => {
                        ...
                        _ => {
                            ...
                            // 初始化 port
                            port.init(clb as u64, fb as u64, &ctbas);
...
pub fn port(ctrl_num: u8, port_num: u8) -> &'static mut HbaPort {
    let list = unsafe { locked_hba_port_list.lock() };
    return list[ctrl_num as usize][port_num as usize];
}   
