...
// 一个简易的napi的实现，这里的代码可以实现关闭中断-轮询收包-开启中断的流程，但不能实现更复杂的机制，包括在收包队列在多个网卡之间的共享以及收包时间在多个网卡之间的轮转等
// 因为目前的协议栈还不支持底层设备向协议栈中提供packet，我们只能在NapiStruct中缓存收到的packet，同时在网卡驱动的receive方法中调用napi对应的receive方法
struct NapiStruct{
    _intr_set: Box<dyn Fn(bool) -> bool>,
    _pkt_recv: Box<dyn Fn() -> Vec<u8>>,
    _nid: u16,
}

impl NapiStruct{
    pub fn intr_set(&self, state: bool) -> bool{
        (*self._intr_set)(state)
    }
    pub fn pkt_recv(&self) -> Vec<u8>{
        (*self._pkt_recv)()
    }
}
