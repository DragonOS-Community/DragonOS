...
// use log::info;
use crate::{
    process::{ProcessManager, namespace::NamespaceOps},
};
...
fn get_process_stat(pid: ProcessId) -> Result<String, SystemError> {
    ...
    let (vsize, rss) = if let Some(addr_space) = pcb.basic().user_vm() {
        let __addr_space_guard = addr_space.read();
        let vsize = 4096 * 1024; 
        let rss = 1024; 
...
fn get_process_ns_mnt(pid: ProcessId) -> Result<String, SystemError> {
    let pcb = ProcessManager::find(pid).ok_or(SystemError::ESRCH)?;
    let mnt_ns = pcb.nsproxy().mnt_ns.clone();
    
    // 使用 namespace 的层级和对象地址生成唯一的 inode 号
    let base_inode = 4026531840u64;
    let level_offset = (mnt_ns.ns_common().level() as u64) * 1000;
    let addr_hash = (Arc::as_ptr(&mnt_ns) as usize as u64) & 0xFFFF;
    let ns_inode = base_inode + level_offset + addr_hash;
...
fn get_process_ns_pid(pid: ProcessId) -> Result<String, SystemError> {
    ...
    // 构造稳定且区分层级的“inode号”（与 NsMnt 一致的思路）
    // 同一 pidns 内所有进程返回相同值；不同 pidns 返回不同值
    let base_inode = 4026531836u64;
    let level_offset = (pid_ns.ns_common().level() as u64) * 1000;
    let addr_hash = (alloc::sync::Arc::as_ptr(&pid_ns) as usize as u64) & 0xFFFF;
    let ns_inode = base_inode + level_offset + addr_hash;
