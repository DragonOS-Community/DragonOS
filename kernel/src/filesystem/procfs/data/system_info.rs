...
pub fn get_system_info(info_type: ProcSystemInfoType) -> Result<String, SystemError> {
        match info_type {
            ...
            ProcSystemInfoType::Kmsg => get_kmsg_info(),
        }
}
...
fn get_mounts_info() -> Result<String, SystemError> {
    ...
    /*
    // 调试信息：打印挂载点数量
    log::info!("get_mounts_info: found {} mount points", mounts.len());
    
    // 打印原始挂载信息用于对比
    for (i, (mp, mfs)) in mounts.iter().enumerate() {
        log::debug!("raw mount[{}]: path='{}', fs_type='{}', has_self_mountpoint={}", 
                 i, mp.as_str(), mfs.fs_type(), mfs.self_mountpoint().is_some());
    }*/
    ...
    for (mp, mfs) in mounts {
        ...
        let source = match fs_type {
            // 特殊文件系统，直接显示文件系统名称
            "sysfs" | "devfs" | "devpts" | "proc" | "tmpfs" | "ramfs" | "rootfs"
        ...
        // 调试信息：打印处理后的挂载条目
        //log::debug!("mount entry: {}", line.trim_end());
...
// 获取内核日志信息
fn get_kmsg_info() -> Result<String, SystemError> {
    unsafe {
        if let Some(kmsg) = crate::filesystem::procfs::kmsg::KMSG.as_ref() {
            let mut kmsg_guard = kmsg.lock_irqsave();
            
            // 预分配缓冲区读取kmsg数据
            let mut buffer = vec![0u8; 8192]; // 8KB缓冲区
            let len = kmsg_guard.read(&mut buffer)?;
