...
impl File {
    ...
    fn new_with_private_data_and_mount_guard(
        ...
        // fsnotify：仅当系统存在 watch 时，解析父目录+子项名快照存入 File，
        // 供内容事件 hook 向父目录 watch 投递子项 IN_MODIFY/ACCESS/OPEN/CLOSE（issue B）。
        // 无 watch 时跳过（零开销）。设备/套接字等非 MountFSInode 解析为 None。
        let inotify_parent = if fsnotify::has_any_watch() {
    ...
    pub fn do_read(
        ...
        // fsnotify：读成功后投递 IN_ACCESS（FMODE_NONOTIFY 短路）。
