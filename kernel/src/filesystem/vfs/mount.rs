...
impl MountFS {
    ...
    /// # Safety
    ///
    /// 此函数使用 unsafe 代码来修改 `MountFS` 的不可变字段 `mount_flags`。
    /// 这样做是安全的，原因如下：
    ///
    /// 1. **并发安全**：调用者必须持有挂载命名空间的锁（如 `mount_list` 的锁），
    ///    这确保了不会有多个线程同时修改同一个 MountFS 的标志。
    ///
    /// 2. **不变量维护**：`mount_flags` 是一个简单的 `MountFlags` 位标志结构体，
    ///    不包含任何引用或指针。修改它不会破坏其他字段的不变量。
    ///
    /// 3. **原子性**：`MountFlags` 的赋值是原子操作（只是复制整数位标志），
    ///    不会出现中间状态。
    ///
    /// 4. **可见性**：由于锁的存在，修改对其他线程是可见的。
    ///
    /// 注意：此函数应该只在挂载命名空间的锁保护下调用。
