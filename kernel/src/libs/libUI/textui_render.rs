use crate::include::bindings::bindings::{font_ascii, video_frame_buffer_info};

use super::textui::{TextuiCharChromatic};
#[allow(dead_code)]
...
pub static mut FB: u64 = 0;
pub static mut BUF_WIDTH: u32 = 0;
...
pub fn textui_render_chromatic(actual_line: u16, index: u16, character: &TextuiCharChromatic) {
    ...
    for i in 0..TEXTUI_CHAR_HEIGHT {
        ...
        let mut addr: *mut u32 = (unsafe { FB } as u32
            + unsafe { BUF_WIDTH } * 4 * (y as u32 + i)
...
// fb:缓冲区起始地址
// num:要重置的像素点数量
pub fn renew_buf(fb:u64,num:u32) {
    let mut addr: *mut u32 = unsafe { FB }as *mut u32;
    for _i in 0.. num {
        unsafe { *addr = 0 };
        unsafe {
            addr = (addr.offset(1)) as *mut u32;
