...
impl AbsHandleMap {
    ...
    pub fn alloc(&self) -> Option<Arc<AbsHandle>> {
        let abs_addr = match ABS_ADDRESS_ALLOCATOR
            .lock()
            .alloc() {
        ...
        return Some(
            Arc::new(
            AbsHandle::new(Arc::from(ads_addr_fmt.as_bytes()))
            )
        ) 
    }
    ...
    pub fn remove(&self, name: String) -> Result<(), SystemError> {
        let abs_addr = match self.look_up(name) {
            Some(addr) => addr,
            None => return Err(SystemError::EINVAL),
        };

        let abs_addr_str = abs_addr.name().;

        let parsed_abs_addr = u32::from_str_radix(&abs_addr, 16).expect("Failed to parse address!");
