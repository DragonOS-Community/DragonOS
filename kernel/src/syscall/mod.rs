...
impl Syscall {
    ...
    pub fn handle(syscall_num: usize, args: &[usize], frame: &mut TrapFrame) -> usize {
        syscall_result(syscall_num, args, frame).unwrap_or_else(|e| e.to_posix_errno() as usize)
    }
...
fn syscall_result(
    ...
    match syscall_num {
        SYS_PUT_STRING => Syscall::put_string(args[0] as *const u8, args[1] as u32, args[2] as u32),
        SYS_OPEN => {
        ...
        SYS_READ => {
            ...
            let mut user_buf = UserBufferWriter::new(buf_vaddr as *mut u8, len, from_user)?;
            let slice = user_buf.buffer::<u8>(0)?;
            Syscall::read(fd, slice)
        }
        SYS_WRITE => {
            let fd = args[0] as i32;
            let buf_vaddr = args[1];
            ...
            let user_buf = UserBufferReader::new(buf_vaddr as *const u8, len, from_user)?;
            let buf = user_buf.read_from_user(0)?;
        ...
        SYS_FORK => Syscall::fork(frame),
        SYS_VFORK => Syscall::vfork(frame),

        SYS_BRK => {
            let new_brk = VirtAddr::new(args[0]);
            Syscall::brk(new_brk).map(|vaddr| vaddr.data())
        }

        SYS_SBRK => {
            let increment = args[0] as isize;
            Syscall::sbrk(increment).map(|vaddr: VirtAddr| vaddr.data())
        }

        SYS_REBOOT => Syscall::reboot(),
        ...
        SYS_WAIT4 => {
            ...
            Syscall::wait4(pid, wstatus, options, rusage)
        }
        ...
        SYS_MKDIR => {
            ...
            if path.is_err() {
                ...
                Syscall::mkdir(path.unwrap(), mode)
            }
        }
        ...
        SYS_NANOSLEEP => {
            ...
            if frame.from_user()
                && (verify_area(virt_req, core::mem::size_of::<TimeSpec>() as usize).is_err()
                    || verify_area(virt_rem, core::mem::size_of::<TimeSpec>() as usize).is_err())
            {
                Err(SystemError::EFAULT)
            } else {
        ...
        SYS_UNLINK_AT => {
            ...
            if frame.from_user() && verify_area(virt_pathname, PAGE_4K_SIZE as usize).is_err() {
                ...
                if pathname.is_err() {
                    ...
                    Syscall::unlinkat(dirfd, pathname.unwrap(), flags)
                }
            }
        }
        SYS_KILL => {
        ...
        SYS_SIGACTION => {
            ...
            let act = args[1];
            let old_act = args[2];
            Syscall::sigaction(sig, act, old_act, frame.from_user())
        }
        ...
        SYS_GETPID => Syscall::getpid().map(|pid| pid.into()),

        SYS_SCHED => Syscall::sched(frame.from_user()),
        SYS_DUP => {
        ...
        SYS_SOCKET => Syscall::socket(args[0], args[1], args[2]),
        SYS_SETSOCKOPT => {
            ...
            if verify_area(virt_optval, optlen as usize).is_err() {
                ...
                Syscall::setsockopt(args[0], args[1], args[2], data)
            }
        }
        SYS_GETSOCKOPT => {
            ...
            if r.is_err() {
                ...
                Syscall::getsockopt(args[0], args[1], args[2], optval, optlen as *mut u32)
            }
        }
        ...
        SYS_CONNECT => {
            ...
            if verify_area(virt_addr, addrlen as usize).is_err() {
                ...
                Syscall::connect(args[0], addr, addrlen)
            }
        }
        SYS_BIND => {
            let addr = args[1] as *const SockAddr;
            ...
            if verify_area(virt_addr, addrlen as usize).is_err() {
                ...
                Syscall::bind(args[0], addr, addrlen)
            }
        }
        ...
        SYS_SENDTO => {
            ...
            if verify_area(virt_buf, len as usize).is_err() {
                ...
                Syscall::sendto(args[0], data, flags, addr, addrlen)
            }
        }
        ...
        SYS_RECVFROM => {
            ...
            if r.is_err() {
                ...
                Syscall::recvfrom(args[0], buf, flags, addr, addrlen as *mut u32)
            }
        }
                ...
                Ok(mut user_buffer_writer) => {
                    match user_buffer_writer.buffer::<crate::net::syscall::MsgHdr>(0) {
                        ...
                        Ok(buffer) => {
                            ...
                            Syscall::recvmsg(args[0], msg, flags)
                        }
                    }
                }
            }
        ...
        SYS_LISTEN => Syscall::listen(args[0], args[1]),
        SYS_SHUTDOWN => Syscall::shutdown(args[0], args[1]),
        SYS_ACCEPT => Syscall::accept(args[0], args[1] as *mut SockAddr, args[2] as *mut u32),
        SYS_GETSOCKNAME => {
            Syscall::getsockname(args[0], args[1] as *mut SockAddr, args[2] as *mut u32)
        }
        SYS_GETPEERNAME => {
            Syscall::getpeername(args[0], args[1] as *mut SockAddr, args[2] as *mut u32)
        }
        SYS_GETTIMEOFDAY => {
            let timeval = args[0] as *mut PosixTimeval;
            let timezone_ptr = args[1] as *mut PosixTimeZone;
            Syscall::gettimeofday(timeval, timezone_ptr)
        }
        SYS_MMAP => {
        ...
        SYS_MUNMAP => {
            ...
            if addr & MMArch::PAGE_SIZE != 0 {
                ...
                Syscall::munmap(VirtAddr::new(addr), len)
            }
        }
        SYS_MPROTECT => {
            let addr = args[0];
            ...
            if addr & MMArch::PAGE_SIZE != 0 {
                ...
                Syscall::mprotect(VirtAddr::new(addr), len, args[2])
            }
        }
        ...
        SYS_GETCWD => {
            ...
            if r.is_err() {
                ...
                Syscall::getcwd(buf).map(|ptr| ptr.data())
            }
        }

        SYS_GETPGID => Syscall::getpgid(Pid::new(args[0])).map(|pid| pid.into()),

        SYS_GETPPID => Syscall::getppid().map(|pid| pid.into()),
        SYS_FSTAT => {
            let fd = args[0] as i32;
        ...
        SYS_FTRUNCATE => {
            ...
            let res = Syscall::ftruncate(fd, len);
