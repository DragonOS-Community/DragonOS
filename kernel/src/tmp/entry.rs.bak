    ...
    fn eq(&self, other: &Self) -> bool {
        ...
        if other.1.is_some() {
            if let Some(enkey) = self.0.upgrade() {
                return enkey.0.lock().name == other.1.unwrap();
            } 
        }else if self.1.is_some() {
            if let Some(enkey) = other.0.upgrade() {
                return enkey.0.lock().name == self.1.unwrap();
            }
        }
        false
    }
...
impl PartialOrd for Keyer {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        ...
        if other.1.is_some() {
            if let Some(enkey) = self.0.upgrade() {
                return Some(enkey.0.lock().name.cmp(&other.1.unwrap()));
            }
        } else if self.1.is_some() {
            if let Some(enkey) = other.0.upgrade() {
                return Some(enkey.0.lock().name.cmp(&self.1.unwrap()));
            }
        }
        Some(Ordering::Less)
    }
...
impl Ord for Keyer {
    fn cmp(&self, other: &Self) -> Ordering {
        ...
        if other.1.is_some() {
            if let Some(enkey) = self.0.upgrade() {
                return enkey.0.lock().name.cmp(&other.1.unwrap());
            }
        } else if self.1.is_some() {
            if let Some(enkey) = other.0.upgrade() {
                return enkey.0.lock().name.cmp(&self.1.unwrap());
            }
        }
        Ordering::Less
    }
...
impl Inode {
    fn new(file_type: FileType, mode: ModeType) -> Inode {
        Inode {
            data: Vec::new(),
    ...
    fn from(file_type: FileType, mode: ModeType, data: usize) -> Inode {
        Inode {
            data: Vec::new(),
...
impl IndexNode for LockedEntry {
        ...
        match ino.into() {
            ...
            ino => {
                let mut key: Vec<String> = entry
                    ...
                    .iter()
                    .filter_map(|(_, value)| {
                        match value.0.lock().inode.0.lock().metadata.inode_id.into() {
                            ino => Some(value.0.lock().name),
                            _ => None,
                        }
                    })
                    .collect();
    ...
    fn mknod(
        ...
        let mut nod_entry = nod.0.lock();
        let mut nod_inode = nod_entry.inode.0.lock();
        
        nod_entry.self_ref = Arc::downgrade(&nod);
        if mode.contains(ModeType::S_IFIFO) {
            nod_inode.metadata.file_type = FileType::Pipe;
            // 创建pipe文件
            let pipe_inode = LockedPipeInode::new();
            // 设置special_node
            nod_entry.special_node = Some(SpecialNodeData::Pipe(pipe_inode));
        } else if mode.contains(ModeType::S_IFBLK) {
            nod_inode.metadata.file_type = FileType::BlockDevice;
            unimplemented!()
        } else if mode.contains(ModeType::S_IFCHR) {
            nod_inode.metadata.file_type = FileType::CharDevice;
            unimplemented!()
        }
