...
QEMU_DRIVE="id=disk,file=${QEMU_DISK_IMAGE},if=none -drive id=ext2-disk,file=../bin/ext2.img,if=none"
QEMU_ACCELARATE=""
QEMU_ARGUMENT=""
QEMU_DEVICES=""
...
if [ ${ARCH} == "i386" ] || [ ${ARCH} == "x86_64" ]; then
    ...
    QEMU_DEVICES_DISK="-device ahci,id=ahci -device ide-hd,drive=disk,bus=ahci.0 -device ide-hd,drive=ext2-disk,bus=ahci.1 "
else
...
QEMU_DEVICES+=" -device ide-hd,drive=ext2-disk,bus=ahci.1 -netdev user,id=hostnet0,hostfwd=tcp::12580-:12580 -device virtio-net-pci,vectors=5,netdev=hostnet0,id=net0 -usb -device qemu-xhci,id=xhci,p2=8,p3=4 " 
# E1000E
