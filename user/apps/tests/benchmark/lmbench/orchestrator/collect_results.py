...
def parse_last_run(serial_path):
    """Return the last run and explicit frame-presence metadata."""
    ...
    for i, ln in enumerate(lines):
        if RUN_BEGIN in ln:
            begin = i
    framed = begin is not None
    if begin is None:
        begin = 0
    meta, summary, metrics = None, None, []
    meta_count = summary_count = 0
    ended = False
    ...
    return meta, metrics, summary, frame


def protocol_errors(meta, metrics, summary, frame):
    errors = []
    if not frame["begin"]:
        errors.append("missing RUN_BEGIN frame")
    if not frame["end"]:
        errors.append("incomplete frame: missing RUN_END")
    if frame["meta_count"] != 1 or meta is None:
        errors.append("frame must contain exactly one valid META")
    if frame["summary_count"] != 1 or summary is None:
        errors.append("frame must contain exactly one valid SUMMARY")
    if summary is not None:
        ...
        if any(summary.get(key) != value for key, value in expected.items()):
            errors.append(f"summary counts do not match metrics: expected {expected}")
    return errors
...
def main():
    ...
    meta, metrics, summary, frame = parse_last_run(args.serial)
    errors = protocol_errors(meta, metrics, summary, frame)
    if errors:
        for error in errors:
            eprint(f"[collect] ERROR: {error}")
        return 2
    if not metrics:
