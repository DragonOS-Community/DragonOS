...
use info::{GAddInfo, GDelInfo, GModInfo, PasswdInfo, UAddInfo, UDelInfo, UModInfo};
use std::{
...
impl GAddCheck {
    ...
    pub fn check(cmd: GroupCommand) -> GAddInfo {
        ...
        if info.groupname.is_empty() {
            ErrorHandler::error_handle(
                "groupname is required".to_string(),
                crate::error::ExitStatus::InvalidArg,
            );
        }
